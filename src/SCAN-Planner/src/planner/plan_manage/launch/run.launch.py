...
def _setup(context):
    ...
    requested_body_pose = LaunchConfiguration("body_pose_topic").perform(context)
    requested_sensor_pose = LaunchConfiguration("sensor_pose_topic").perform(context)
    requested_cloud = LaunchConfiguration("cloud_topic").perform(context)
    requested_planning_frame = LaunchConfiguration("planning_frame").perform(context)
    start_sport_bridge = _as_bool(LaunchConfiguration("start_sport_bridge").perform(context))
    require_localization = _as_bool(
        LaunchConfiguration("require_localization").perform(context)
    )
    ...
    default_init = (-19.0, 1.0, 0.25) if navi_mode == 1 else (-5.5, 5.5, 0.5)
    init_values = []
    for name, default in zip(("init_x", "init_y", "init_z"), default_init):
        value = LaunchConfiguration(name).perform(context)
        init_values.append(default if value == "" else float(value))

    if is_real:
        body_pose = requested_body_pose or "/Odometry"
        sensor_pose = requested_sensor_pose or "/Odometry"
        cloud = requested_cloud or "/cloud_registered"
        planning_frame = requested_planning_frame or "world"
    ...
    if controller_mode == "open_loop":
        ...
        if is_real and start_sport_bridge:
            actions.append(
                Node(
                    package="scan_planner",
            ...
            actions.append(
                Node(
                    ...
                    parameters=[
                        ...
                        common,
                        {
                            "init_x": init_values[0],
                            "init_y": init_values[1],
                            "init_z": init_values[2],
                            "publish_tf": False,
                        },
                    ],
                    remappings=[
...
def generate_launch_description():
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("planning_frame", default_value=""),
            DeclareLaunchArgument("start_sport_bridge", default_value="true"),
