...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  const V3D acc_direction = mean_acc.normalized();
  const Eigen::Quaterniond body_to_world =
      Eigen::Quaterniond::FromTwoVectors(acc_direction, V3D::UnitZ());

  // Level camera_init with gravity while leaving yaw at its initial value.
  // At rest the accelerometer measures upward specific force, so body +g
  // is aligned with world +Z and gravity is fixed along world -Z.
  init_state.rot = body_to_world.normalized().toRotationMatrix();
